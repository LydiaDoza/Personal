...
*	bool CityInList()
*		Returns true if the city is found in the list
...
Graph<V, E>& Graph<V, E>::operator=(const Graph<V, E> & rhs)
{
	if (this != &rhs)
	{
		...
		for (Vertex<V, E> v : rhs.m_vertices)
			// create new vertices
			m_vertices.emplace_back(v.m_data);
...
void Graph<V, E>::InsertEdge(const V & from, const V & to, const E & edge, int weight)
{
	...
	else // both are valid
	{
		for (Edge<V, E> & e : itF->m_edges)
			...
			{
				cout << itF->m_data << " with edge " << edge << " --- ";
...
* Entry: 	V from, V to, E edge, int weight
...
/**********************************************************************
* Purpose:	checks if city is in list.
* Entry: 	const V & data
* Exit:		true or false
************************************************************************/
template <typename V, typename E>
bool Graph<V, E>::CityInList(const V & data) const
{
	bool found = false;
...
/**********************************************************************
* Purpose:	Returns list of vertices.
* Entry:	None.
* Exit:		list<Vertex<V, E>> of vertices
************************************************************************/
template <typename V, typename E>
list<Vertex<V, E>> Graph<V, E>::GetVertices() const
{
	return m_vertices;
}
